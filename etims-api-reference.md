...
- **Async endpoints** return `{ "syncBatchId": "<uuid>" }`. Confirm resolution via the read endpoints or dashboard sync monitoring → [Tracking Results](./etims-tracking-results.md).
...
- For valid field **codes** (tax category, units, packaging, payment, classification), see [Codes & Values](./etims-codes.md).
...
> Connecting a company is normally done in the **dashboard**; the `provision` endpoint is for programmatic onboarding only. See [Connecting a Company](./etims-provisioning.md).
...
`itemType` determines stock behavior: **`GOODS`** are stock-tracked (sales deduct inventory), **`SERVICE`** items are non-stock (sales ignore inventory). See [Goods vs Services](./etims-catalog.md#goods-vs-services-stock-vs-non-stock).
...
Returns `{ syncBatchId }`. Details: [Catalog](./etims-catalog.md).
...
Returns `{ syncBatchId }`. Details: [Sales & Credit Notes](./etims-sales.md).
...
Returns `{ syncBatchId }`. Details: [Stock](./etims-stock.md).
...
Synchronous; returns the connection result. `409` if the company is already connected. Details: [Connecting a Company](./etims-provisioning.md).
