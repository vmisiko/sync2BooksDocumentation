---
title: "Get Started with eTIMS"
description: "This guide takes you from zero to a signed KRA receipt for your first sale."
---
...
Go to the [Sync2Books Dashboard](https://sync2books.com/dashboard) and sign up. The form is a short two-step wizard: your personal details, then your organization (company name, industry, team size).
...
the `X-API-Key` header; an HMAC signature (`X-Signature` + `X-Timestamp`) is optional.
The full mechanics — key formats, your client secret, and how to sign requests — live
once in [Core concepts → Authentication](/concepts/authentication).
</Note>
...
> Where do the KRA PIN, device serial, and branch ID come from? From the business's KRA OSCU onboarding — see **[the third-party integrator guide](/ETIMS_OSCU_INTEGRATION_AS_THIRD_PARTY)**. Sandbox values are fine while developing.
>
> Prefer to automate onboarding for many companies? There's an optional **provisioning API** too — see [Provisioning → Advanced](/etims-provisioning#advanced-provision-via-api).
...
> **Want to try these instantly?** Import the [eTIMS Postman Collection](/etims-postman) — every request below is in it, with variables for your key and IDs.
...
>
> (The **add stock** step is for **goods** only — **services** are non-stock, so skip it. See [Goods vs Services](/etims-catalog#goods-vs-services-stock-vs-non-stock).)
...
> **`classificationCode` is effectively required** — without a valid KRA item classification code the register is rejected. See [Catalog](/etims-catalog#classification-codes).
...
> Selling a **service** (`itemType: SERVICE`)? Services are **non-stock** — skip this step and go straight to the sale. See [Goods vs Services](/etims-catalog#goods-vs-services-stock-vs-non-stock).
...
The sale is then marked **accepted** and carries a KRA receipt number (`ETR-…`). See [Tracking Results](/etims-tracking-results) for how to know when it's done.
...
- [Postman Collection](/etims-postman) — run every request in seconds
- [Provisioning](/etims-provisioning) — the dashboard flow (and optional API)
- [Catalog](/etims-catalog) — classification codes, the REGISTERED lifecycle
- [Sales & Credit Notes](/etims-sales) — document states, prerequisites, receipts
- [Stock](/etims-stock) — adjust and transfer
- [Tracking Results](/etims-tracking-results) — the async model
