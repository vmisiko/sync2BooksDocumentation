...
description: "This is where compliance pays off: you submit a sale, the platform pushes it to KRA over OSCU, and KRA returns a signed receipt (ETR number). You can then..."
---
...
1. **The item is synced to eTIMS.** A registered-but-not-synced item is rejected with *"Item has not been synced to eTIMS"*. → [Catalog](/etims-catalog)
2. **For goods, there is enough stock.** Selling a **`GOODS`** item **deducts inventory**, so it fails with *"Insufficient stock"* if there isn't enough on hand — add stock first → [Stock](/etims-stock). **`SERVICE` items are non-stock**: skip this entirely; they can be sold right after syncing. See [Goods vs Services](/etims-catalog#goods-vs-services-stock-vs-non-stock).
...
This is **asynchronous** — you get a `syncBatchId` immediately; the receipt arrives once KRA accepts. See [Tracking Results](/etims-tracking-results).
...
For valid `receiptTypeCode`, `paymentTypeCode`, and `invoiceStatusCode` values, see **[Codes & Values](/etims-codes)**.
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| `Item has not been synced to eTIMS (missing etimsItemCode)` | Item registered but not synced | Run catalog sync → [Catalog](/etims-catalog) |
| `Insufficient stock: Have 0, tried to deduct N` | No inventory | Add stock → [Stock](/etims-stock) |
| Sale not accepted | A prerequisite failed | Check the failure reason → [Tracking Results](/etims-tracking-results) |
| Wrong line `id` | Used `externalId` instead of the item id | Use the item `id` from Sync2Books |

## Read next

- [Stock](/etims-stock) — keep inventory in sync
- [Tracking Results](/etims-tracking-results) — know when a sale is accepted
- [API Reference](/etims-api-reference#sales) — full field reference
