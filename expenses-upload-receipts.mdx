...
description: "Attach files (receipts, invoices, documents) to synced expense transactions"
---
...
```bash
curl -X POST \
  "https://api.sync2books.com/connections/{connectionId}/syncs/{syncId}/transactions/{transactionId}/attachments" \
...
curl -X GET "https://api.sync2books.com/sync/batches/sync-uuid" \
  -H "X-API-Key: sk_production_your_api_key"

# Step 3: Upload attachment using syncId and transactionId (expense ID)
curl -X POST \
  "https://api.sync2books.com/connections/{connectionId}/syncs/sync-uuid/transactions/exp-001/attachments" \
  -H "X-API-Key: sk_production_your_api_key" \
  -F "file=@receipt.jpg" \
...
def upload_attachment(connection_id, sync_id, transaction_id, file_path, api_key):
    url = f"https://api.sync2books.com/connections/{connection_id}/syncs/{sync_id}/transactions/{transaction_id}/attachments"
...
* [Create transactions](/expenses-create-transactions) - Create and sync expenses
* [Sync transactions](/expenses-sync-transactions) - Track sync status
* [Configure customer](/expenses-configure-customer) - Connect accounting systems
