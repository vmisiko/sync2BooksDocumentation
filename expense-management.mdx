...
description: "This guide covers how to create, sync, and manage expenses in Sync2Books."
---
...
See [Sync & Monitoring](/sync-and-monitoring) for detailed status tracking.
...
```bash
curl -X GET "https://api.sync2books.com/expenses/{expenseId}" \
  -H "X-API-Key: sk_production_your_api_key"
```
...
```bash
curl -X GET "https://api.sync2books.com/expenses/connection/{connectionId}" \
  -H "X-API-Key: sk_production_your_api_key"
```
...
If the sync fails, check the sync batch status for error details. See [Sync & Monitoring](/sync-and-monitoring) for error handling.
...
- **[Sync & Monitoring](/sync-and-monitoring)** - Learn how to track sync status
- **[Attachments](/attachments)** - Upload files to expenses
